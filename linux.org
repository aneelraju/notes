...
**** The login shell
  + The login shell
    + it's command line interpreter defaults to bash (bourne again shell)
    + users may be allowed to change their login shells with 'chsh' command
    + The changes shell should be one of the shells in /etc/shells. The administrator can edit this file
  ...
  + The /etc/shadow file
    ...
    + the file contains the following nine fields:
      ...
      + account expiration date. it represents the number of days since 1/1/1970. If left blank, the account will never expire
    + The /etc/group file
      + contains the current groups on the system and the users listed in each one. each group is represented by a line. Fields are separated by colons, no spces are allowed
      + the line contains the following fields
        + group name
        + encrypted password or a placeholder
        + gid
        + member users
      + the password placeholder is used if the group has a password set (using 'gpasswd' command). however, it's highly unusual
    + creating new users
      + the 'useradd' command is used to add new users to the system. it does following tasks
      ...
      + users can be added using, instead, the 'vipw' and 'vipw -s' which would edit /etc/passwd and /etc/shadow files respectively
      + you must assign a password for the new user before he/she can use the account. Passwords are assigned using the 'passwd loginame' command
        + default '!' is present in the password field until set password. it means user account is locked
    + Environment files
      ...
      + those startup files are placed in '/etc/skel' directory. they get copied to the user's home directory upon account creation
        + % cd /etc/skel
        + % ls -la; # contains startup files
      + the '/etc/profile' file get's executed before any startup files are. it is a good place to store system-wide environment variables. however, this file can be easily overriden by the users
**** The useradd command
  + The 'useradd' command
    + it uses the files '/etc/login.defs' and '/etc/default/useradd'
    ...
    + switch -D prints the defaults that 'useradd' uses when creating new users
  ...
  + 'useradd' options
    ...
    + % useradd -c "System Operator" -d /opt/systemoperator -g admins -G operators -s /bin/ksh operuser; # example command
    + % vi /etc/passwd; # check for user operuser
    + % su - operuser; # invokes ksh
    + % finger operuser; # check info
  ...
  + Adding multiple users in one go
    ...
    + the file is formatted the same way as the '/etc/passwd' file, except that the x placeholder in the password fields is replaced with the actual password, in clear text
      + % echo '
        usera:abc123:510:510:First User:/home/usera:/bin/bash
        userb:abc123:511:511:Second User:/home/userb:/bin/bash
        userc:abc123:512:512:Third User:/home/userc:/bin/bash
        ' > users.txt
      + chmod 400 users.txt; # only read permission to you for security
      + % newusers users.txt
      + % vi /etc/passwd; # users are added
      + % vi /etc/shadow; # passwords are added
  ...
  + Deleting user accounts
    ...
    + in ubuntu there is the 'deluser' script, which is a wrapper to 'userdel'
      + it uses the configuration file /etc/deluser.conf to provide the following features (they can be turned on or off)
  ...
  + users locking and unlocking
    ...
    + locking the account means putting an asterisk (*) or an exclamation mark before the encrypted password field in the '/etc/shadow' file as this destroys the hash. Unlocking the password removes those characters
      + % usermod -L usera; # locks usera account
      + % vi /etc/shadow; # check '!' mark
      + % usermod -U usera; # unlocks userb account

*** storage
**** storage
     + more than just "disks"
       + long ago, storage used to refer to one or more disks attached to the machine and used
         to store the operating system and user files
       + the hard disk (or SSD) is still the dominant part of storage but a lot of layers are built
         on top of it like:
       ...
       + storage has evolved from being just a number of disks attached to the machine to a whole
         compartment of disks, with separate poewr, and processor, connected to the machines through
         lightning fast fiber channel cables, or several gigabit ethernet cables (iSCSI) called storage
         area networks (SAN)
     + what is a disk ?
       + it is made up of several rotating platters coated with magnetic film
       + reading and writing is done through sliding heads that move back and forth just above the
         surface of the platter but never touch it
       + when a read operation is requested, the head must move the position above the appropriate
         track. The time consumed by this operation is called seek delay
       + it then has to wait for the desired sector to pass under the head as the platter rotates,
         rotationaly latency
       + a number of tracks that are at the same distance from the spindle are called a cylinder.
         Because the arm does not need to move to read the data, it is read much faster
     + Hard Disk Drives
       + the rotational speed is measured in RPM (revolutions per minute). you can see speeds like 
         7200rpm, 10000rpm or 15000rpm depending on the type of storage used
       + hard disk drives suffers frequent failures, that's why technologies like RAID were introduced
         to help avoid data loss in the event of an HDD failure
       + a failure may occur as a result of bad blocks on platter surface, or the damage in the
         mechanical components like the spindle or the heads
       + disk life expectancy is measured in MTBF (Mean Time Between Failures). However, this should
         not be used to expect failure rate over a long term, as the manufacture selects the most
         reliable phase of a drive
...
**** TCP/IP overview
     ...
     + TCP/IP skeleton
       + transmission control protocol/internet protocol (TCP/IP), a set of network protocols stacked over each other and designed to work with each other
         + application layer: it is where the network-aware application resides. For e.g ssh, ftp
         ...
         + link layer: where network drivers operate. it also contains the ARP (Address Resolution Protocol), which is responsible for translating logical address (IP) to hardware addresses (MAC)
         + physical layer: the newtork medium where data travels (copper wires, radio waves ... etc)
...
**** the Link layer
  + The Link Layer
    + To enable TCP/IP to support different kinds of networks (Ethernet, token ring ..etc) there had to be an abstraction b/w different layers. The link layer for e.g is responsible for managing the low level harware details of the specific network. It is where the drivers decide the way data will be sent and received. Other layers do not and should not care about the network medium as it is abstracted by the link layer
    + the link layer determines the max length of the packet, which is the unit of data transferred over the network. This is governed by the hardware limits and the protocol specification
    + For e.g. an Ethernet packet is about 1500 bytes, a PPP modem link packet is b/w 512 and 576 and a point-to-point WAN link could be b/w 1500 and 4500 bytes
  + How do packets know their destinations ?
    + several methods are used in conjuction: MAC addressing, IP (v4 or v6) addressing, and hostnames
    + MAC address
      + it is a 6B address to the network adapter at time of manufacture. It is unique amoung all network adapters
      + It represented in hex format in pairs separated by colons. Eor e.g: 00:0a:95:9d:68:16. The first 3B identify the vendor. For eg. 00:0a:95 for Apple Computers Inc. A comprehensive list can be found at http://iana.org/assignments/ethernet-numbers
      + Although MAC address were intended to be unique, many adapters offer a way to change them, especially if you are a vitual machine
    + IP addressing
      + An IP address consists of 32 bits that can be represented as 4 numbers separated by dots. For eg. 192.168.1.100. The ARP protocol on the link layer translates IP address to the physical MAC address to enable communication
    + Hostnames
      + Human-readable names of machines. Translation from hostnames to IP addresses can be done through several systems from machine hosts files to LDAP databases or DNS systems
        + % vi /etc/hosts; # host names
        + % lsof -:80; # apache server running port 80
        + % vi /etc/services; # list of services and corresponding port numbers, check for port 80
      + The application running on the host uses a 16-bit number called a part so that the operating system can correctly direct messages to the intended recipient application
  + IP addressing
    + An IP address is divided into network (subnet) and the physical host
    + when an IP address starts with 127 this means it is referring to the loopback network. If the IP address is 127.0.0.1, it is refererring to the current host (the loopback address)
    ...
    + A netmask is used to specify the network and host portion of an IP address. For e.g 192.168.5.10/24 is a class C IP address
    + Classfull IP addressing can be summarized as follows
      + Class:A; Range:1-127; Possible hosts:16,777,214; Possible networks: 126; Netmask: 8
      + Class:B; Range:128-191; Possible hosts:65,534; Possible networks:16,384; Netmask: 16
      + Class:C; Range:192-223; Possible hosts: 254; Possible networks: 2,097,152; Netmask: 24
**** classless inter domain routing (CIDR)
  + Classless inter domain routing (CIDR)
    + Default subnet classes wasted a lot of addresses. Note that 127 is reserved for loop back addresses
    ...
    + Class:A; Possible Network bits: 8-30; Possible subnets: 1-4,194,304; Possible hosts: 2-16,777,214
    + Class:B; Possible Network bits: 16-30; Possible subnets: 1-16,384; Possible hosts: 2-65,534
    + Class:C; Possible Network bits: 24-30; Possible subnets: 1-64; Possible hosts: 2-254
  + Private address spaces and NAT
    ...
    + NAT uses port forwarding to enable internal addresses to receive requests from the outside
      + e.g <realip>:80 -> port 80 forwarded to <private ip>
    + NAT is not and cannot replace a firewall solution
**** routing
  + Routing
    + It is the method by which a packet finds it's way from source IP to destination IP through a set of one or more networks (or subnets)
    ...
    + Routing table can be displayed using the 'netstat -r' command. -n can be added to avoid DNS lookups:
      ...
      + % netstat -nr; # kernel routing table
      + % tracepath www.google.com
      + % ping www.google.com
    + a route can be added statically for e.g
  ...
  + ARP: Address Resolution Protocol
    + It is used for translating the IP address of a machine to the physical MAC address of the adapter so that data can flow b/w source and destination devices
    + If the target m/c is not on the same network, the routing table is consulted to determine the next hop and ARP is used to find the router's MAC address
    + When the source machine wants to send data to the target machine, it broadcasts a message on Ethernet asking other machines on the network, the target machine replies with it's MAC address. The request includes the IP address and MAC address of the source machine so that the target one does not have to do the same broadcast message to reply
...
**** Netfilter - The Linux firewall
  + Netfilter or iptables?
    ...
    + Not only is it used for filtering incoming and outgoing packets, but can also be used in situations like NAT
  ...
  + The chains
    ...
    + They can be explained as follows:
      ...
      + Output: when packet is leaving your box, just after the process generates it
      + Postrouting: right before the packet leaves the interface
      + Prerouting: right after the packet enters the network interface
    + By definition, those chains are available to the table according to the table's role. For e.g. it is meaningless to target the postrouting chain in the input table
  + The tables
    + iptables consists of three tables used to organize the hook points (chains) on the kernel to capture packets in the desired point in path
  ...
  + The packet journey in the action
    ...
    + iptable rules must have least restrictive at top and most restrictive at bottom
  + The targets
...
**** The LOG action
  + The LOG action
    ...
    + you can use it to determin whether or not a packet has passed for failed a specific chain. e.g iptables -A INPUT -j LOG
    + by examining the '/var/log/messages' you will know whether or not the packets have been blocked or not
    + % iptables -vnL
    + % iptables -vnL --line-numbers
    + % iptables -I OUTPUT 2 -j LOG
    + % ping 192.168.0.103
  ...
  + I don't want to be pinged
    ...
    + % iptables -A INPUT -p icmp --icmp-type echo-request -j REJECT
      + from other host % ping 192.168.0.106; # ping rejected
    + % iptables -A INPUT -p icmp --icmp-type echo-request -j DROP
      + from other host % ping 192.168.0.106; # ping rejected without message
  + The usual configuration
    ...
    + for e.g. the below configuration is for a system that allows only SSH and HTTP TCP connections
    ...
    + % iptables -A INPUT -j DROP
    + % iptables -vnL
    + % nc -l 8000; # netcat command, opens up arbitrary port
      + % telnet 192.168.0.106 80; # due to firewall, accepted
      + % telnet 192.168.0.106 22; # accepted
      + % telnet 192.168.0.106 8000; # cannot connect, 8000 port is opened by netcat
    + % iptables -F; # iptables flushed
    + % nc -l 8000
      + % telnet 192.168.0.106; # connected, as iptables are flushed
      + % Hello; # appears on host m/c
  + Viewing, editing, and saving rules
    + you can use 'iptables -nvL' to view all the configured rules. The line numbers are important here to be able to make changes
    + you can insert a rule in a specific location using the -l switch, but you must specify a line number. For e.g. I want to block all trafic from subnet 172.25.22.0/24 and if I have traffic from other sources, I want only ssh connections allowed
      + 'iptables -A INPUT -s 172.25.22.0/24 -j DROP'
      + 'iptables -A INPUT -p tcp --dport 22 -j ACCEPT'
      + 'iptables -A INPUT -j DROP'
    + Then I installed a webserver, I want the previous configuration to stay the same, but just add web traffic to the allowed list. Consulting iptables -vnL, I want this rule to be added to line 2
      + 'iptables -I INPUT 2 -p tcp --dport 80 -j ACCEPT'
    + if you want to delete a rule use the -D switch
      + 'iptables -D INPUT 2'
    + you must save the iptables configuration to persis reboots
    ...
    + % iptables -vnL
    + % iptables -vnL --line-numbers
    + % iptable -D INPUT 1; # delete
    + % iptables -vnL --line-numbers
    + % service iptables save
    + % vi /etc/sysconfig/iptables
**** IP tables configuration
  + Iptables configuration
    + Iptables can be started and stopped by using the service command just like other services on the system: 'service iptables start/stop'
    ...
    + The iptables command itself has a large number of options and possible combinations. However, you needn't study all the possible syntaxes for your day to day system administration
    + % vi /etc/sysconfig/iptables; # you can rules here
    + % service iptables status; # status of iptables
    + % service iptables stop; # stop iptables
    + % chkconfig --list iptables
    + % man iptables; # has lots of options and combinations
  + I want to block that IP!
    + most basic usage of iptables is to block traffic from a specific IP address: 'iptables -A INPUT -s 192.168.0.103 - j DROP'
    + the filter table is assumed by default if not explicitly specified. we could, however mention it like: 'iptables -t filter -A INPUT -s 192.168.0.103 - j DROP'
    + Then comes the action argument. The -A is for add, this adds the rule to the end of the chain. you can use -I to instert a rule at a specific place (using line numbers). Use 'iptables -vnL' to display the rules and their line numbers. The action is followed by the chain. In our case it's the INPUT one
    ...
    + % ipconfig -a; # two network cards
    + % iptables -A INPUT -s 192.168.56.101 -j drop
      + From remote m/c % ping 192.168.56.102; # no response and message
    + % iptables -F; # flush iptables
    + % iptables -A INPUT -s 192.168.56.101 -j REJECT
      + From remote m/c % ping 192.168.56.102; # connection rejected and log message
    + % iptables -vnL; # gives iptables, same as iptable -t filter -vnL
    + % iptables -t filter -vnL --line-numbers; # gives line number of the rule
  + Let's block web traffic
    + you can block traffic targetting a specific port number. e.g. HTTP web traffice 'iptables -I INPUT -p tcp --dport 80 -j DROP'
    ...
    + To block clients from specific ip: 'iptables -I INPUT -p tcp --dport 80 -s 192.168.0.103 -j DROP'
    + % lsof -i:80; # check http running
    + % iptables -F
    + % iptables -A INPUT -p tcp --dport 80 -j REJECT; # block tcp traffic
    + % iptables -A INPUT -p udp --dport 80 -j REJECT; # block udp traffic
    + % iptables -t filter -vnL --line-numbers; # 2 rules are added
      + In another terminal
        + % telnet 192.168.0.106 80; # connection refused
    + % iptables -F
      + In another terminal; % telnet 192.168.0.106 80; # connection accepted
    ...
    + % iptables -vnL
      + From client 192.168.0.103; % telnet 192.168.0.106 80; # connection accepted
      + From different client; % telnet 192.168.0.106 80; # connection rejected
  + Controlling connection state
    + in very tight environment, outgoing connections are also filtered. But this may cause problems when an incoming connection needs to send response
    + e.g. you may want to limit outgoing ssh connections to a specific host, but allow this host to establish connection with you
    + you can use the --state predicate as 'iptables -A INPUT -p tcp --dport 22 -s 192.168.0.103 -m state --state NEW,ESTABLISHED -j ACCEPT'
    ...
    + % iptables -A OUTPUT -j REJECT
      + % from another m/c, % ssh root@192.168.0.106; # connection rejected
    + % iptables -I OUTPUT -m state --state ESTABLISHED -j ACCEPT; # accept only connections that are established earlier and reject new connections
      + % from another m/c, % ssh root@192.168.0.106; # connection accepted
      + % ping 192.168.0.103; # connection rejected
**** Lab: Configure a Linux box to act as a 12:17 NAT device
  + LAB: configure a linux box to act as a a NAT device
    + our lab consists of two machines
      + m/c 1: will act as the NAT device. it has 2 network interfaces one connected to the host (the outside world) and the other is connected to a private network
      + m/c 2: has only one network card that is connected to the private network. this m/c is not reachable by the host
    + our prequisites: we want to enable ssh access to m/c 2 through m/c 1 port 2222. m/c 1 will use iptables to forward the packets to m/c 2 on port 22 nad masquerade the returning packets to make it appear as if they are originating from m/c 1
    + procedure (on m/c 1)
      + enable kernel routing on m/c 1 by editing '/etc/sysctl.conf' and makking net.ipv4.ip_forward = 1. Apply changes using 'sysctl -w'
      + use iptables to enable port forwarding: 'iptables -t nat PREROUTING -A -p tcp -d 192.168.0.106 --dport 2222 -j DNAT --to 192.168.56.1:22'
      + use iptables to masquerade returning packets: 'iptables -t nat -A POSTROUTING -o eth0 -j MASQUERADE'
      + test the results by connecting from the host to m/c 1 through ssh specifying port 2222. you should find yourself connecting to m/c 2
      + m1> % ifconfig -a; # shows network cards
      + m1> % vi /etc/sysctl.conf
        + set net.ipv4.ip_forward = 1
      + m1> % sysctl -m; # write the config
      + m1> % sysctl -p; # check the config
      + m1> % iptables -F
      + m1> % iptables -t nat -A PREROUTING -p tcp --dport 2222 -j DNAT --to 192.168.56.101:22
      + m1> % iptables -t net -A POSTROUTING -o eth0 -j MASQUERADE
      + m1> % iptables -t nat -vnL
      + m1> % iptables -t net -A POSTROUTING -o eth1 -j MASQUERADE
      + m1> % ipconfig eth0
      + m2> % ssh -p 2222 192.168.0.106; # login to host directory (using m/c 1 as nat device)
      + h> % set -o vl
      + h> % route add default gw 192.168.96.102
      + h> % ping 8.8.8.8
...
**** DNS - Domain Name System
  + Basically, DNS is ...
    + A way/server/service to translate IP addresses to human-friendly names. For e.g. development.mycompany.com is much more readable and intuitive than 223.123.96.35. It also provides some clue about the role of this m/c
    + A distributed system. That is, DNS data is scattered among a large number of servers. Each one manages the records of its own network. For e.g. of mycompany.com will have records of all the hosts using this doman. A DNS server will answer the requests of clients that need to resolve names of its hosts, and it will also query other DNS servers if it doesn't have this info yet
  ...
  + Resource Records
    ...
    + They are stored in forward and reverse files. The first is used to map hostnames to IP addresses while the second is to do the opposite
    + The forward file is named after the domain name e.g. mycompany.com while the reverse file is named after the after the IP address 213.123.96.35.rev
    + In the forward file each record contains: hostname, record type, ip address
    + If the host name in a line is similar to he previous one, it can be omitted
    + In the reverse file, the hostnames are written to be associated with their IP addresses
  + DNS hierarchy and delegation
    + A domain address typically consists of two or more fragments seperated by dots. The left most fragment is called the top level domain (tld) e.g. .com, .net, .edu and .org are all TLD names
...
** book - linux fundamentals - paul cobbaut
*** linux intro
    + links
      + https://en.wikipedia.org/wiki/Dennis_Ritchie
      + https://en.wikipedia.org/wiki/Ken_Thompson
      + https://en.wikipedia.org/wiki/BSD
      + https://en.wikipedia.org/wiki/Comparison_of_BSD_operting_systems
      + https://en.wikipedia.org/wiki/Richard_Stallman
      + https://en.wikipedia.org/wiki/IBM_AIX
      + https://en.wikipedia.org/wiki/HP-UX
      + https://en.wikipedia.org/wiki/Linux_Torvalds
    ...
    + choosing a linux distro
      + red hat enterprise - you are manager and you want a good support contract
      + centos - you want red hat without the support contract from red hat
      + fedora - you want red hat on your laptop/desktop
      + linux mint - you want a personal graphical desktop to play movies, movies and games
...
*** man pages
    + sample commands
      + % man whois; # man command
      + % man syslog.conf; # man configfile
      + % man syslogd; # man daemon
      + % man -k syslog; # man -k (or apropos) shows lit of man pages containing a string
      + % whatis route; # description of a man page
      + % whereis -m whole; # location of man page
      + % man man; # show sections of man page
        ...
        + 6 - games
        + 7 - misc (including macro pages and conventions), e.g. man(7)
        + 8 - system administration commands (usually only for root)
        + 9 - kernel routines (non standard)
      + % man passwd; # opens first manual found
      + % man 5 passwd; # opens a page from section 5
      + % mandb; # update man pages
*** working with directories
    + sample commands
      + % pwd ; # print working directory
      + % cd /etc; # change current directory
      ...
      + % ls -lh; # display file details with size in human readable format
      + % mkdir mydir; # create directory
      + % mkdir -p mydir2/mysubdir2/threedirsdeep; # create sub directories
      + % rmdir mydir; # remove directory
      + % rmdir -p test42/subdir; # remove sub-directories
      + % pushd /lib
      + % pushd /proc
      + % popd
      + % popd
*** working with files
    + sample commands
      + % file pic33.png; # file utility determines the file type
      + % man 5 magic; # look at magic man page
      + % file -s /dev/sda; # for special files
        + % file -s /proc/cpuinfo
      + % touch file42; # create an empty file
      + % touch -t 200505050000 SinkoDemay; # timestamps file
      + % rm BigBattle.txt; # remove file
      + % rm -i file33; # ask before removing file
      + % rm -rf test; # remove file forcefully and recursive
      + % cp file1 file2; # copy file1 to file2
      + % cp file1 dir2; # copy file1 into dir2
      + % cp -r dir42/ dir33; # copy dir42 to dir33
      + % cp file42 file43 file44 dir42/; # copy file42,file43 and file44 to dir42
      + % cp -i file1 file2; # ask before copy
      + % mv file1 file2; # renames files
      + % mv dir33 backup; # rename directories
      + % mv -i file1 file2; # ask before rename
      + % rename 's/\.txt/\.png' *.txt; # debian: rename files using regex
      + % rename 's/file/document/' *.png; # debian
      + % rename .conf .backup *.confg; # centos: rename usage
      + % reanem one ONE *; # centos: rename usage
*** working with file contents
    + sample commands
      + % head /etc/passwd; # display first 10 lines
      + % head -4 /etc/passwd; # display first 4 lines
      + % head -c14 /etc/passwd; # display first 14 bytes
      + % tail /etc/services; # will display the last ten lines of a file
      + % tail -3 /etc/services; # display last 3 lines
      + % cat /etc/resolv.conf; # copy std input to std output
      + % cat part1 part2 part3; # dispaly part1 part2 part3
      + % cat all; # display all files
      + % cat > winter.txt; # create file
        > It is very cold today! <ctrl+d>
      + % cat > hot.txt <<stop; # custom end marker 'stop'
        > It is hot today!
        > Yes it is summer
        > stop
      + % cat winter.txt > cold.txt; # copy files
      + % tac count; # cat backwards
      + 'more' command is usefule for displaying files that take up more than one screen
        + <spc> to see the next page
        + 'q' to quit
        + some people prefer the 'less' command to more
        + % more /etc/services
      + % strings /bin/ls; # strings command can display readable ascii strings found in (binary) files
      + % ls -lrS /etc; # finds biggest file in /etc
      + % cat /etc/passwd >> tailing.txt; # append to file
      + % mv tailing.txt tmp.txt; % cat /etc/passwd tmp.txt > tailing.txt; # trick for a file to be preceded by another file
*** the linux file tree
    + % man hier; # info about file system hierarcy
    + http://www.pathname.com/fhs - filesystme hierarchy standard
    + % ls /; # root directory
    + binary directories
      + /bin - contains binaries for use by all users
        + like /bin/cat, /bin/date
      + other /bin directories
        + found in subdirectories like 
          + in home directories - /home/xyz/bin
          + in some application when installed directly from sources will put themselves in /opt - /opt/samba/bin
      + /sbin - contains binaries to configure the operating system
        + many of system binaries require root privilege
      + /lib - binaries found in /bin and /sbin often use shared libraries
        + /lib/modules - linux kernel loads kernel modules form /lib/modules/$kernel-version
        + /lib32 and /lib64 - 32-bit and 64-bit directories
          + % file /lib32/libc-2.5.so; # 32-bit file type
          + % file /lib64/libcap.so.1.10; # 64-bit file type
          + ELF (Executable and Linkable Format) is used in almost every unix-like operating system since System V
      + /opt - store optional software, like software from outside distribution repository
        + /bin, /lib, /etc - subdirectories within /opt/$packagename/
          + e.g /opt/wp/bin, /opt/wp/man
    + configuration directories
      + /boot - contains all files needed to boot the computer, these files don't change often
        + /boot/grub
          + /boot/grub/grub.cfg - defines boot menu that is displayed before the kernel starts
      + /etc - machine-specific configuration files (historically called 'etcetera' but today called as 'Editable Text Configuration')
        + % ls /etc/*.conf - many times the name of a configuration file is same as application, daemon or protocol with .conf extension
        + /etc/init.d/ - scripts to start and stop daemons
        + /etc/X11/ - graphical dispaly is driven by software from X.org foundation
        + /etc/skel/ - skeleton directory /etc/skel is copied to the home directory of a newly created user like .bashrc script
        + /etc/sysconfig/ - contains lot of red hat linux config files (not mentioned in FHS)
          + % ls /etc/sysconfig/
            + % ls /etc/sysconfig/firstboot - tell red hat agent not to run at boot time
              + if you want to run red hat setup agent in next reboot, then simply remove this file and run 'chkconfig --level 5 firstboot on'
              + red hat setup agent allows you to install the latest updates, create a user account, join the red hat network and more
              + /etc/sysconfig/harddisks - contains some parameters to tune the hard disks
              + /etc/sysconfig/hwconf - hardware detected by kudzu (software from red hat for automatic discovery and configuration of hardware)
              + /etc/sysconfig/keyboard - keyboard type and keymap table are set here
                + % cat /etc/sysconfig/keyboard
                + check the manual pages of keymaps(5), dumpkeys(1), loadkeys(1) and directory /lib/kbd/keymaps
    + data directories
      + /home - users can store personal or project data under /home
        + % ls /home; # list users home directories
        + % ls -d /home/paul/.*
      + /root - default location for personal data and profile of the root user
      + /srv - data that is served by your system
        + FHS allows locating cvs, rsync, ftp and www data in this locaiton
        + FHS also approves administrative naming in /srv like /srv/project55/ftp and /srv/sales/www
        + on Sun Solaris /export is used for this purpose
      + /media - directory serves as a mount point for removable media devices such as CD-ROM, digital cameras and various usb-attached devices
        + % ls /media/;
      + /mnt - should be empty and hsould only be used for temporary mount points
      + /tmp - applications and users should use /tmp to store temporary data when needed
        + data stored in /tmp may use either disk space or RAM, both of which are managed by operating system
        + never use /tmp to store data that is important or which you wish to archive
    + in memory directories
      + /dev - device files in /dev appear to be ordinary files, but are not actually located on hard disk
        + % ls dev/sd*; # common physical devices
        + % ls /dev/hd*; # common physical devices
        + /dev/tty - represents a terminal or console attached to the system
        + /dev/pts - when typing commands in a terminal that is part of a graphical interface like Gnome and KDE
        + /dev/null - special device, it has unlimited storage, but nothing can be retrieved
      + /proc - another special directory, appearing as ordinary files, but not taking up disk space
        + it's actually a view of kernel, what kernel manages
        + % ls /proc; 
        + % file /proc/cpuinfo; # shows empty
        + % cat /proc/cpuinfo; # info about CPU
        + % cat /proc/interrupts; # displays the interrupts
        + % ls -lh /proc/kcore; # physical memory, do not try to cat this file, instead use a debugger
      + /sys - directory contains kernel information about hardware (hot plug devices)
        + linux uses 'sysfs' to support usb and firewire hot plug devices. see manual pages udev(8)
        + http://linux-hotplug.sourceforge.net/
    + /usr unix system resources
      + /usr/bin - contains lot of commands
        + % ls /usr/bin | wc -l
      + /usr/include - contains general use include files for C
        + % ls /usr/include/
      + /usr/lib - directory contains libraries that are not directly executed by users or scripts
        + % ls /usr/lib | head -7
      + /usr/local - directory can be used by an adminstrator to install software locally
        + % ls /usr/local/
        + % du -sh /usr/local/
      + /usr/share - contains architecture independent data
        + % ls /usr/share/  wc -l
        + % du -sh /usr/share
        + % ls /usr/share/man - directory typically contains /usr/share/man for manual pages
        + % ls /usr/share/games/ - static game data
      + /usr/src - directory is the recommended location for kernel source files
        + % ls -l /usr/src
    + /var variable data - files that are unpredictable in size, such as log, cache and spool files
      + /var/log - central point to contain all log files
        + % ls /var/log
      + /var/log/messages - typical first file to check when troubleshooting on red hat (and derivaties)
        + /var/log/syslog for debian and ubuntu
          + % tail /var/log/messages
      + /var/cache - directory can contain cache data for several applications
        + % ls /var/cache
      + /var/spool - directory typically contains spool directories for main and cron, but also serves as parent directory for other spool files (like print spool files)
      + /var/lib - contains applications state information
        + /var/lib/rpm/ - red hat keeps files pertaining to rpm
      + /var/...
        + /var also contains process id files in /var/run (or /run) and temporary files taht survive a reboot in /var/tmp and info about file locks in /var/lock
    + % dd if=/dev/zero of=zeros.txt count=1 bs=100; # dd will copy one times a block of size 100 bytes to zeroes.txt
      + /dev/zero is a linux special device, can be considered a source of zeroes. you cannot send something to /dev/zero but you can read zeroes from it
    + % od zeros.txt
    + % dd if=/dev/random of=random.txt count=1 bs=100; od random.txt
      + /dev/random acts as a random number generator
