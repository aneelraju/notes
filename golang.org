...
**** Incrementor With Channels
 + % cd work/22_go-routines/09_channels/07_incrementor; % go fmt main.go; % go run -race main.go
**** Deadlock Challenge
 + % cd work/22_go-routines/10_deadlock-challenges/01_deadlock-challenge; % go fmt main.go; % go run -race main.go
 + % cd work/22_go-routines/10_deadlock-challenges/02_deadlock-solution; % go fmt main.go; % go run -race main.go
 + % cd work/22_go-routines/10_dealock-challenges/03_deadlock-challenge; % go fmt main.go; % go run -race main.go
 + % cd work/22_go-routines/10_deadlock-challenges/04_deadlock-challenge; % go fmt main.go; % go run -race main.go
 + % cd work/22_go-routines/10_deadlock-challenges/05_deadlock-solution; % go fmt main.go; % go run -race main.go
**** Factorial Challenge
 + % cd work/22_go-routines/11_factorial-challenge/01_challenge-description; % go fmt main.go; % go run -race main.go
 + % cd work/22_go-routines/11_factorial-challenge/02_challenge-solution; % go fmt main.go; % go run -race main.go
**** Pipeline Pattern
 + https://blog.golang.org/pipelines - go concurrency patterns
 + % cd work/22_go-routines/12_channels_pipeline/01_sq-output; % go fmt main.go; % go run -race main.go
**** Factorial Challenge Redux
 + % cd work/22_go-routines/12_channels_pipeline/02_sq-output; % go fmt main.go; % go run -race main.go
 + % cd work/22_go-routines/12_channels_pipeline/03_challenge-description; % go fmt main.go; % go run -race main.go
**** Factorial Challenge Redux Solution
 + https://github.com/GoesToEleven/GolangTraining/blob/master/22_go-routines/12_channels_pipeline/04_challenge-solution/README.md
 + % cd work/22_go-routines/12_channels_pipeline/04_challenge-solution/01_original-solution; % go fmt main.go; % go run -race main.go
 + % cd work/22_go-routines/12_channels_pipeline/04_challenge-solution/02_another-solution; % go fmt main.go; % go run -race main.go
**** Fan Out/ Fan In Pattern - Overview
 + https://blog.golang.org/pipelines - check for fanout and fanin
 + fanout - some channel -> multiple funcs reading from that channel until it's closed
 + fanin - multiple channels writing to the same channel -> some channel
**** Fan In Pattern
 + https://talks.golang.org/2012/concurrency.slide#25 - rob pike google talk slide
 + % cd work/22_go-routines/13_channels_fan-out_fan-in/01_boring; % go fmt main.go; % go run -race main.go
**** Fan Out/ Fan In - Example
 + % cd work/22_go-routines/13_channels_fan-out_fan-in/02_sq-output; % go fmt main.go; % go run -race main.go
 + % cd work/22_go-routines/13_channels_fan-out_fan-in/03_sq-output_variation; % go fmt main.go; % go run -race main.go
*** Concurrency Challenges
**** Fan Out/Fan In - Challenge
 + https://github.com/golang/link - linter for go source code
  + go get -u github.com/golang/lint/golint
  + % golint; # lint one file
  + % golint ./...; # lint multiple file
 + % cd work/22_go-routines/13_channels_fan-out_fan-in/04_challenge-description; % go fmt main.go; % go run -race main.go
**** Fan Out/Fan In - Solution
 + % cd work/22_go-routines/13_channels_fan-out_fan-in/05_challenge-solution; % go fmt main.go; % go run -race main.go
**** Fan Out/Fan In - Challenge: Factorial
 + % cd work/22_go-routines/13_channels_fan-out_fan-in/06_challenge-description; % go fmt main.go; % go run -race main.go
**** Fan Out/Fan In - Solution: Factorial
 + % cd work/22_go-routines/13_channels_fan-out_fan-in/07_challenge-solution; % go fmt main.go; % go run -race main.go
**** Deadlock Challenge
 + % cd work/22_go-routines/13_channels_fan-out_fan-in/08_challenge-description; % go fmt main.go; % go run -race main.go
**** Deadlock Solution
 + % cd work/22_go-routines/13_channels_fan-out_fan-in/09_challenge-solution; % go fmt main.go; % go run -race main.go
**** Incrementor Challenge Revisited
 + % cd work/22_go-routines/14_incrementor-challenge/01_description; % go fmt main.go; % go run -race main.go
**** Incrementor Solution
 + % cd work/22_go-routines/14_incrementor-challenge/02_solution; % go fmt main.go; % go run -race main.go
...
**** Improving your code with Golint
  ...
  + super simple highlighter - chrome plugin to highlight pages
  + https://github.com/golang/lint - golang linter
    + % go get -u github.com/golang/lint/golint
  + % cd work/23_error-handling/01_golint/01_before; % golint ./...
  + % cd work/23_error-handling/01_golint/02_after; % golint ./...
**** Handling Errors & Logging Errors to a File
  + https://golang.org/ref/spec#Program_initialization_and_execution - Program initialization and execution
  + https://godoc.org/log - log package
  + https://godoc.org/os - os package
  + % cd work/23_error-handling/02_err-not-nil/01_fmt-println; % go fmt main.go; % go run main.go
  + % cd work/23_error-handling/02_err-not-nil/02_log-println; % go fmt main.go; % go run main.go
  + % cd work/23_error-handling/02_err-not-nil/03_log-set-output; % go fmt main.go; % go run main.go
**** Four Common Ways to Handle Errors
  + https://godoc.org/os#Exit - exit
  + https://godoc.org/builtin - builtin package
  + https://golang.org/pkg/ - list of golang packages
  + % cd work/23_error-handling/02_err-not-nil/04_log-fatalln; % go fmt main.go; % go run main.go
  + % cd work/23_error-handling/02_err-not-nil/05_panic; % go fmt main.go; % go run main.go
**** Custom Errors - Creating Values of Type error
  ...
  + https://golang.org/src/errors/errors.go - errors source code
  + % cd work/23_error-handling/03_custom-errors/01_errors-new; % go fmt main.go; % go run main.go
**** Idiomatic Error Handling
  + https://golang.org/src/bufio/bufio.go - bufio source code, check custom errors
  + % cd work/23_error-handling/03_custom-errors/02_errors-new_var; % go fmt main.go; % go run main.go
**** Providing Context with Errors
  + https://godoc.org/fmt#Errorf - fmt package Errorf function
  + % cd work/23_error-handling/03_custom-errors/03_fmt-errorf; % go fmt main.go; % go run main.go
  + % cd work/23_error-handling/03_custom-errors/04_fmt-errorf_var; % go fmt main.go; % go run main.go
**** Providing Even More Context with Errors
  + https://golang.org/doc/effective_go.html#errors
  + % cd work/23_error-handling/03_custom-errors/05_custom-type; % go fmt main.go; % go run main.go
...
**** Error Handling Review & Resources
  + resources
    ...
    + https://www.goingo.net/2014/11/error-handling-in-go-part-ii.html
